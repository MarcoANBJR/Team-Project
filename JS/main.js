...
function membersList () {
  ...
  peoples.forEach(element => {
    ...
    for (let i = 0; i < element._certificates.length; i++) {
      let liCertificates=document.createElement('li')
      liCertificates.textContent= `Certificado: ${element.certificates(i).liContent}`       
      ulMembers.appendChild(liCertificates)   
    }    
